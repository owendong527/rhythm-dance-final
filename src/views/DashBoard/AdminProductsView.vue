<template>
  ...
  <div class="container">
        ...
        <tbody class="fs-5">
          <tr v-for="item in products" :key="item.id">
            <td class="d-none d-sm-table-cell">
              <img class="img-fluid" :src="item.imageUrl" alt="" width="100"></td>
            <td >{{ item.title }}</td>
            ...
            <td class="text-center">
              <div class="btn-group flex-sm-row flex-column">
                ...
                btn-sm mb-sm-0 mb-2" @click="openModal('edit', item)">
                  編輯
                </button>
                <button type="button" class="btn btn-outline-danger
                btn-sm" @click="openModal('del', item)">
                  刪除
                </button>
              </div>
            </td>
    ...
    <!-- 分頁元件 Ｅ -->
    <ProductModal :product="tempProduct"
    :is-new="isNew" @update-product="updateData"  ref="productModal" />
    <DelModal :item="tempProduct" @del-item="removeData" ref="delModal"/>
  </div>
</template>
